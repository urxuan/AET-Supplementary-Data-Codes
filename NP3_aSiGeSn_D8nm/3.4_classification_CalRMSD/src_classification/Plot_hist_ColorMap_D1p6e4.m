function [temp_model, temp_atomtype] = Plot_hist_ColorMap_D1p6e4(rec, curr_model, curr_type, classify_info,xscale,yscale)
...
y_up = round(max(hist_inten_plot))/1;% 1p6e5 No Probe
...
figure; clf;box on;
%set(gcf,'Position',[150,150,700,105]) ver1
set(gcf,'Position',[150,150,700/2.2,105*1.4])
hold on
for i=1:Num_species
...
%lgd = legend('Non-atoms','Si','Ge','Sn');
%lgd = legend('Non-atoms','Si:70.1%','Ge:99.60%','Sn:100%');% dose1p6e4,probe
%lgd = legend('Non-atoms','Si:61.3%','Ge:98.98%','Sn:99.96%');% dose1p6e4,No probe
lgd = legend('Non-atoms','Si: 61.3%','Ge: 99%','Sn: 100%');% dose1p6e4,No probe

%lgd = legend('Non-atoms','Si:82.86%','Ge:99.96%','Sn:100%');% dose1p6e5,probe
%lgd = legend('Non-atoms','Si:75.12%','Ge:99.92%','Sn:100%');% dose1p6e5,No probe
lgd.Box = 'off'; 
