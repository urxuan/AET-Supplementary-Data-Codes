function [temp_model, temp_atomtype] = initial_class_kmean_sub(rec, curr_model, classify_info,y_up)
...
%y_up = round(max(hist_inten_plot));
...
%set(gca,'FontSize',12,'FontName', 'Arial','FontWeight','bold','linewidth',2);
  ...
  %  ylabel('# atoms');
    ...
    ylim([0 y_up]);
    xlim([0 max(sum(box_inten_plot,1))]);
end
else
