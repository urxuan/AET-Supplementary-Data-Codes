...
Recon_filename = ['Resire_aSi2nm_FosCetr_Dose1p6e4_cov35pm_BM3D_S0p25N200.mat'] % 
ourputstring_prefix = ['polyn_tracing_aSi2nm_FosCetr_Dose1p6e4_cov35pm_BM3D']
Rec_ourput_prefix = ['Rec_aSi2nm_FosCetr_Dose1p6e4_cov35pm_BM3D_S0p25N200']
%% load reconstructed 3D volume
Dsetvol0 = importdata(Recon_filename);
Dsetvol0 = Dsetvol0.reconstruction;
Dsetvol0=imgaussfilt(Dsetvol0,1);
Mask=importdata('Mask_aSi_cov35pm.mat');
Dsetvol=Mask.*Dsetvol0;
%figure();img(Dsetvol,'colormap','gray')
...
xxi = xxi(ov:end); yyi = yyi(ov:end); zzi = zzi(ov:end);
...
Res     = 0.195/ov;  minDist  = 1.93/Res;     SearchRad = ov;
...
maxNum      = min(5000,length(sortInd));
...
%
 ...
 toc    
%% Apply a cutoff based on intensity. The cutoff should be set slightly smaller than the minimum bond length, accounting for RMSD.
for i = 1:size(maxXYZ,1)
    ...
    %%
save(sprintf('%s.mat',ourputstring_prefix),'PosArr','TotPosArr','CoeffArr','Orders','exitFlagArr','SearchRad','maxXYZ','minDist','goodAtomTotPos');
%%
