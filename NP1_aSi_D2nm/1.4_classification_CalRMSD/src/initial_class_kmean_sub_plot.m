function [temp_model, temp_atomtype] = initial_class_kmean_sub_plot(rec, curr_model, classify_info)
...
y_up = round(max(hist_inten_plot)*1.1);
for i = 1:Num_species+1
    if i == 1
        subplot(Num_species+1,1,1);
        hist(sum(box_inten_plot,1),separate_part,'LineWidth', 2);
        title(sprintf('boxsize %d',halfSize*2+1));
h = findobj(gca,'Type','patch');
% h.FaceColor = [0 0.5 0.5];
%h.EdgeColor = 'w';
%h.EdgeColor = [0.0 100/255 246/255];

h.FaceColor = [0.4 0.7 1];
%h.FaceColor = [0.0 102/255 255/255];
%h.FaceColor = [63/255 141/255 255/255];
h.FaceAlpha = 1;
set(gca,'Layer','top')
